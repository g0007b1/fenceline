...
`fenceline` installs guards that are supposed to stop an AI coding agent from doing damage. A way around a guard is therefore a security issue for everyone who relies on it.
...
- **Hook config tampering**: a way for an agent to edit `.fenceline/config.json`, the hook scripts, or the runtime's hook config through normal tools.
