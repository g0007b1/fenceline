...
```
bin/agent-ready.js        CLI: scan | init | refresh | check | doctor | triage | skill | presets | uninstall
src/
  ...
  init.js                 profile + preset → .agent-ready/config.json, hooks, rules, commands, managed docs
  render.js               {{var}} rendering + <!-- agent-ready:managed --> block merging
...
skill/agent-ready/        SKILL.md + references — the procedure an agent follows to finish the setup
...
3. Hooks read `.agent-ready/config.json` on every call and keep per-session state in `.agent-ready/state/<id>.json`; denies and asks go to `.agent-ready/audit.log`. No daemon, no cache. An unparsable config denies everything.
...
- **Namespaced output.** Everything written is under `.agent-ready/`, prefixed `agent-ready-`, merged into an existing hook config, or inside managed markers. `uninstall` is the exact inverse.
...
| add a slash command | `templates/commands/agent-ready-<name>.md` + `COMMANDS` in `src/init.js` |
