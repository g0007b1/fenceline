...
npx agent-ready scan --json > /tmp/agent-ready-profile.json
```
...
```bash
npx agent-ready init -y --runtime <cursor,claude,codex,gemini,copilot> [--profile strict|balanced|light] [--only hooks,rules,commands,docs,domain-docs,templates] [--preset <id>] [--siblings ../a,../b] [--protected-branches a,b] [--base-branch <b>] [--branch-prefix <p>] [--require-tests]
```

Use `-y` (you are not a terminal user; the wizard would wait for input) and pass the answers from the interview as flags. Use every runtime the team uses. Read the report; every line is a file you now own. Later tweaks: `npx agent-ready config set <key> <value>`.

### 4. Fill the judgement parts

The generator leaves `TODO(agent-ready)` markers and skeleton domain docs on purpose — the tool cannot know the product. You can. Fill, in this order:
...
- **Rules** (`agent-ready-*` in the runtime's rules dir): edit the stack rule to match real conventions you saw in the code (alias names, component folder shape, theme hook name, module layout). Concrete beats generic.
...
- **Every ban must be enforceable or explicit.** If you add "never touch X" to AGENTS.md, either add the path to `.agent-ready/config.json → denyWrite` (then `npx agent-ready doctor`) or mark it "convention, not enforced".
- **Numbers over adjectives** in domain docs: "list can reach 5k rows" beats "large list".

### 5. Verify

```bash
npx agent-ready check     # hooks wired, checks runnable, docs present, TODOs left
npx agent-ready doctor    # dry-run the guards + simulate a session through the stop-hook
```
...
- one line on how to refresh (`npx agent-ready refresh` after stack / convention changes) and how to remove (`npx agent-ready uninstall`).

## When refreshing an existing setup

- `npx agent-ready refresh` updates only managed blocks, hooks, rules and commands; human text stays.
- Re-run `scan` first and diff `.agent-ready/profile.json` against the previous one: new risk deps or new fragile zones are the reason to touch the docs.
