...
function check(root, { runChecks = true } = {}) {
  ...
  say(fs.existsSync(cfgPath), '.agent-ready/config.json present');
  if (!fs.existsSync(cfgPath)) { console.log('\nRun `npx agent-ready init` first.\n'); return false; }
  const cfg = JSON.parse(fs.readFileSync(cfgPath, 'utf8'));
  if (cfg.version !== 2) warn(`config version ${cfg.version} — run \`npx agent-ready refresh\` to upgrade`);
  for (const f of HOOK_SCRIPTS) say(fs.existsSync(path.join(root, '.agent-ready', 'hooks', f)), `hook script ${f}`);
  ...
  const todo = ['CLAUDE.md', 'docs/README.md', ...fs.existsSync(path.join(root, 'docs')) ? fs.readdirSync(path.join(root, 'docs')).filter((f) => f.endsWith('.md')).map((f) => 'docs/' + f) : []]
    .filter((f, i, arr) => arr.indexOf(f) === i && fs.existsSync(path.join(root, f)) && fs.readFileSync(path.join(root, f), 'utf8').includes('TODO(agent-ready)'));
  if (todo.length) warn(`unfilled TODO(agent-ready) sections in: ${todo.join(', ')} — run /agent-ready-domain-doc or fill by hand`);
  if (!cfg.checks.length) warn('no lint / type-check commands configured — the stop-hook cannot enforce anything. See `agent-ready scan` warnings.');
