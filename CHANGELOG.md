...
- `--no-review`, `--no-docs-sync`, `--check "<cmd>"`, `--protected-branches`, and **`agent-ready config get|set`** for any knob afterwards; `agent-ready runtimes` shows what is detected.
...
- **Fail-closed on tooling**: `.agent-ready/**` and every runtime hook config are protected from edit tools and shell; an unparsable config denies everything.
- **Per-session state** (`.agent-ready/state/<id>.json`); `SessionStart(compact|resume)` no longer wipes it. **Audit log** of every deny / ask.
...
- **Slash commands** installed per runtime: `/agent-ready-review`, `/agent-ready-pr`, `/agent-ready-handoff`, `/agent-ready-domain-doc`, `/agent-ready-triage`.
- **`agent-ready triage`** — deterministic auto / needs-ac / human verdict from the repo's own risk zones, protected paths, fragile zones and keyword heuristics; scriptable exit codes.
- **`agent-ready doctor`** now also simulates a full session through the stop-hook on a scratch state file, and tests sibling-repo guards for the repo's actual siblings.
- **`agent-ready uninstall`** — exact inverse of init; `--docs` strips managed blocks.
- **`agent-ready skill`** — installs the skill into `.agents/skills` / `.claude/skills`.
